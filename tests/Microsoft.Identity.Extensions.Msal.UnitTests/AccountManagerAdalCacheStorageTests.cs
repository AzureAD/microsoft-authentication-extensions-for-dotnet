...
namespace Microsoft.Identity.Extensions.Msal.UnitTests
{
    ...
    public class AccountManagerAdalCacheStorageTests : IDisposable
    {
        ...
        {
            var store = new IdentityServiceTokenCacheStorage(CacheFilePath, instanceName: null, logger: this.logger);
        ...
        {
            var store = new IdentityServiceTokenCacheStorage(CacheFilePath, instanceName: null, logger: this.logger);
            var store2 = new IdentityServiceTokenCacheStorage(CacheFilePath, instanceName: null, logger: this.logger);
        ...
        public void TestRootSuffixWithNullSuffixCacheAndRegistry()
        {
            ...
            store = new IdentityServiceTokenCacheStorage(null, instanceName: string.Empty, logger: this.logger);

            string cacheFilePath = Path.Combine(localDataPath, @"msal.cache");
            Assert.Equal(store.CacheFilePath, cacheFilePath, ignoreCase: true);

            store = new IdentityServiceTokenCacheStorage(null, instanceName: null, logger: this.logger);
            Assert.Equal(store.CacheFilePath, cacheFilePath, ignoreCase: true);
        }
        ...
        {
            var store = new IdentityServiceTokenCacheStorage(CacheFilePath, instanceName: "Exp", logger: this.logger);
            Assert.Equal(store.CacheFilePath, CacheFilePath, ignoreCase: true);
            string localDataPath = SharedUtilities.GetDefaultArtifactPath();

            store = new IdentityServiceTokenCacheStorage(null, instanceName: "Exp", logger: this.logger);
